...
formula1 <- as.formula("survival_months ~ race + marital_status + t_stage + n_stage + x6th_stage + 
                       differentiate + grade + a_stage + estrogen_status + progesterone_status+ln_tumor + sqrt_examined+  age")
...
# Our 1st global model for the predicts without colinearity and not normal predicts
model_global_bin = lm(status ~ race + marital_status + t_stage + n_stage + x6th_stage + 
                       differentiate + grade + a_stage + estrogen_status + progesterone_status + ln_tumor + sqrt_examined + age, newbc2)
...
binmodel_partial_1 = lm(status ~ marital_status + t_stage + n_stage + differentiate + grade + estrogen_status + progesterone_status + sqrt_examined + age, newbc2)

summary(binmodel_partial_1)

# 2nd binary partial test with all in-significant covariates
binmodel_partial_2 = lm(status ~ race + x6th_stage + a_stage + ln_tumor, newbc2)

summary(binmodel_partial_2)

# 3rd binary partial test with some significant in 2nd test added 
binmodel_partial_3 = lm(status ~ marital_status + t_stage + n_stage + differentiate + grade + estrogen_status + progesterone_status + sqrt_examined + age + a_stage + ln_tumor, newbc2)
...
```
From the backward elimination, the best model with lowest AIC of -8726.68 is: status ~ marital_status + t_stage + n_stage + x6th_stage + differentiate + grade + estrogen_status + progesterone_status + sqrt_examined + age. 
...
```
From the forward selection, the best model with lowest AIC of -8726.53 which is higher than that in backward elimination, so we will use the backward elimination model. 
...
```
- From the stepwise regression, we have the lowest AIC of -8726.68 which is the model of status ~ marital_status + t_stage + n_stage + x6th_stage + differentiate + grade + estrogen_status + progesterone_status + sqrt_examined + age which is the same result from backward elimination, so we will deicde to use this model for further criterion procedure. 
