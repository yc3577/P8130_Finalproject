...
The lambda value of tumor size, regional node examined, and regional node positive are all close to 0. Therefore, we should use log transformation of those three variables.
...
summary(model)
```
Based on the above indicator test summary, we delete grade and x6th_stage because their output was NA in the output linear model.  
